...
export default function ClaimLanesPage() {
  ...
  return <section className="view workspace-view claim-lanes-view">
    <PageHeading eyebrow="Automatisation" title="Files LIVE & REPLAY" subtitle="Deux chaînes de claim indépendantes, pilotées et observées depuis PostgreSQL." actions={<><DataSourceBadge label="POSTGRES"/><button className="secondary-btn" onClick={() => query.refetch()}><Icon name="refresh" size={14}/>Actualiser</button></>} tabs={<PageTabs items={[{ label: "Cockpit", to: "/operations", end: true }, { label: "Agents IA", to: "/operations/agents" }, { label: "AI Context", to: "/operations/ai-context" }, { label: "Files GPT", to: "/operations/claim-lanes" }, { label: "Portfolio Risk", to: "/operations/portfolio-risk" }, { label: "Exécution", to: "/operations/execution" }, { label: "Observabilité", to: "/operations/observability" }, { label: "Incidents", to: "/operations/incidents" }]}/>}/>

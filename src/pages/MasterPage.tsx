...
import { DecisionDeskStrip, DeskReading } from "@/components/deskCards";
import { PageHeading, TechnicalDetails } from "@/components/operations";
...
function MasterWorkspace({ initialData }: { initialData: DeskSession }) {
  ...
  return <section className="view master-document-page">
    ...
    <DecisionDeskStrip
      data={data}
      focusLabel="Chemin attendu"
      focusValue={data.master.expectedPath[0] || data.liveBrief.nextAction}
      focusDetail={data.master.failurePath[0] ? `Échec à surveiller · ${data.master.failurePath[0]}` : data.master.summary}
    />

    ...
    for (const item of items) {
      ...
      const result = await this.contextGate.evaluateAndPersist({
        ...
        policy: {
          enabled: true,
          mode: "SHADOW",
          policy_version: item.contextGate?.policy_version || "us-grains-market-context-prefilter-v1",
          model_policy_version: item.contextGate?.policy_version || item.marketContextSnapshotId || "deterministic-context-prefilter-v1",
        },
        advisory: {
          recommendation: item.contextGate?.recommendation || "TAKE",
          confidence: item.contextGate?.confidence ?? 0.5,
          risk_multiplier: item.contextGate?.risk_multiplier ?? 1,
          ...
          model_ref: item.contextGate?.model_ref || (item.marketContextSnapshotId ? `market-context-snapshot://${item.marketContextSnapshotId}` : "policy://deterministic-context-prefilter-v1"),

...
export function frontTimeSeriesContracts({ view, execution = {}, strategy = {}, risk = {}, marketSeries = null, performance = null, nowIso = currentUtc() }) {
  ...
  const contracts = [
    seriesContract({ seriesId: "market.ohlcv", label: "OHLCV marché", unit: "price", source: "market_candles", availability: hasMarket ? "KNOWN" : marketSeries?.availability || "UNAVAILABLE", reason: hasMarket ? "" : marketSeries?.reason || "Aucune bougie clôturée disponible pour cette période.", schema: "ohlcv_series_v1", cursor: marketSeries?.page?.nextCursor || null }),

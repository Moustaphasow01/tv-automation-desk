...
import { SystemClock, parisOffset, toParisIso } from "@tv-automation/desk-time";
...
import {
  ...
  replayIdempotencyDocumentId,
  replayRequestHash,
  resolveIdempotentReplayResult,
} from "./replay-concurrency.js";
import {
...
export class PersistentDeskStore {
  ...
  async getLatestAsiaOpenPack({ date } = {}) {
    if (date) {
      return compactPack(await this.getDeskPack({ pack_id: `${date}_asia_open` }));
    }

    const docs = await this.#listDocuments(COLLECTIONS.deskPacks, 50);
    const ready = docs
      .filter((pack) => pack.session === "asia_open" && pack.status === "ready")
      .sort(comparePackFreshness);
    if (!ready.length) {
      throw new Error("latest_asia_open_pack_not_found");
    }
    return compactPack(ready[0]);
  }

  async listDeskPacks({ date_from, date_to, session = "asia_open", status = "ready", limit = 100 } = {}) {
    const docs = await this.#listDocuments(COLLECTIONS.deskPacks, Math.max(50, Math.min(Number(limit) || 100, 500)));
  ...
  async getDeskPack({ pack_id, pack_build_id, mode = "live", include_draft = false }) {
    const logicalPack = await this.#getDocument(COLLECTIONS.deskPacks, pack_id);
    const resolvedBuildId = resolveRequestedPackBuildId({ logicalPack, pack_build_id, mode });
    if (resolvedBuildId) {
      const build = await this.#getDocument(COLLECTIONS.deskPackBuilds, resolvedBuildId)
        .catch(() => logicalPack.pack_build_id === resolvedBuildId ? logicalPack : null);
      if (!build) throw deskError("PACK_NOT_FOUND", `Pack build not found: ${resolvedBuildId}.`, { pack_id, pack_build_id: resolvedBuildId });
    ...
    if (!include_draft && logicalPack.status !== "ready") {
      throw new Error("pack_not_ready");
    }
  ...
  async getDataset({ pack_id, pack_build_id, dataset, as_of_utc, mode = "live", format = "json", max_rows = 1000 }) {
    ...
    if (!ref) {
      throw new Error(`dataset_not_found:${dataset}`);
    }
  ...
  async getMarketLevels({ pack_id, instrument }) {
    ...
    if (!levels) {
      throw new Error(`market_levels_not_found:${instrument}`);
    }
  ...
  async getMacroCalendar({ date, pack_id, pack_build_id, as_of_utc, mode = "live", importance_min = "medium" } = {}) {
    ...
    if (pack.pack_build_id) {
      const dataset = await this.getDataset({ pack_id: pack.pack_id, pack_build_id: pack.pack_build_id, dataset: "macro_calendar", as_of_utc: cutoffUtc, mode, max_rows: 5000 });
      return scopedMacroResponse(pack, sanitizeMacroActualsAtCutoff(dataset.rows || [], cutoffUtc), { date: date || pack.date, importance_min });
    }
    const normalized = normalizeMacroDataset(await this.#readStorageText(ref.storage_path), ref, { date: date || pack.date, importance_min });
    return cutoffUtc ? { ...normalized, events: sanitizeMacroActualsAtCutoff(normalized.events, cutoffUtc) } : normalized;
  }
  ...
  async getNewsDigest({ date, session = "asia_open", pack_id, pack_build_id, as_of_utc, mode = "live" } = {}) {
    ...
    if (!ref) {
      return optionalNewsDigestUnavailable({ date: date || pack.date, session });
    }
    const cutoffUtc = as_of_utc || resolvePackCutoffUtc(pack);
    if (pack.pack_build_id) {
      const dataset = await this.getDataset({ pack_id: pack.pack_id, pack_build_id: pack.pack_build_id, dataset: "news_digest", as_of_utc: cutoffUtc, mode, max_rows: 5000 });
      return scopedNewsResponse(pack, filterNewsAtCutoff(dataset.rows || [], cutoffUtc), { date: date || pack.date, session });
    }
    const normalized = normalizeNewsDataset(await this.#readStorageText(ref.storage_path), ref, { date: date || pack.date, session });
  ...
  async createOrchestratedReplayDay(args = {}) {
    const tick = this.clock.now();
    const baseRun = buildOrchestratedReplayRunDoc(args, tick);
    const creationHash = replayRequestHash("create_orchestrated_replay", args);
    const existing = await this.#getDocument(COLLECTIONS.deskReplayRuns, baseRun.backtest_id).catch(() => null);
    if (existing) {
      if (existing.creation_idempotency_key === args.idempotency_key && existing.creation_request_hash === creationHash) {
        return replayCreationResult(existing, null, { idempotent_replay: true });
      }
      throw deskError("IDEMPOTENCY_CONFLICT", `Replay run already exists: ${baseRun.backtest_id}.`);
    }
    const pack = await this.getDeskPack({ pack_id: baseRun.pack_id, pack_build_id: baseRun.pack_build_id, mode: "replay" });
    assertRunPackScope(baseRun, pack);
    const contracts = await this.getActiveContracts();
    const run = pinReplaySources(baseRun, pack, contracts, creationHash, tick);
    ...
    const timeline = replayTimelineEvent(replayRun, step, {
    ...
    const result = replayCreationResult(replayRun, step);
    if (typeof this.persistence.createReplayRun === "function") {
      ...
      if (replayRun.automation_enabled) {
        return { ...committed.result, automation: await this.driveReplayAutomation({ backtest_id: replayRun.backtest_id }) };
      }
      return committed.result;
    }
    await this.#setDocument(COLLECTIONS.deskReplayRuns, replayRun.backtest_id, replayRun);
    await this.#setDocument(COLLECTIONS.deskReplaySteps, step.step_id, step);
    await this.#setDocument(COLLECTIONS.deskReplayTimeline, timeline.event_id, timeline);
    if (replayRun.automation_enabled) {
      return { ...result, automation: await this.driveReplayAutomation({ backtest_id: replayRun.backtest_id }) };
    }
    return result;
  }
  ...
  async prepareReplayMasterBundle(args = {}) {
    const tick = this.clock.now();
    const run = await this.#getDocument(COLLECTIONS.deskReplayRuns, args.backtest_id);
    const begin = await this.#beginReplayMutation(run, args, {
    ...
    const steps = selectBacktestSteps(await this.#listDocuments(COLLECTIONS.deskReplaySteps, 500).catch(() => []), run.backtest_id);
    const preparation = resolveReplayMasterPreparation(steps, run, args.step_id, tick);
    const step = preparation.step;
    const replanContext = preparation.is_replan
      ? buildReplayReplanContext(run, preparation, {
          master: selectLatestReplayMaster(await this.#listDocuments(COLLECTIONS.deskReplayMasterAnalyses, 500).catch(() => []), run.backtest_id),
          thesis: selectReplayActiveThesis(await this.#listDocuments(COLLECTIONS.deskReplayActiveTheses, 500).catch(() => []), run.backtest_id),
          setups: selectReplayScopedSetups(await this.#listDocuments(COLLECTIONS.deskReplaySetups, 500).catch(() => []), run.backtest_id),
          monitor: selectReplayMonitors(await this.#listDocuments(COLLECTIONS.deskReplayMonitors, 500).catch(() => []), run.backtest_id)[0] || null,
          position: selectReplayPositions(await this.#listDocuments(COLLECTIONS.deskReplayPositions, 500).catch(() => []), run.backtest_id)[0] || null,
        })
      : null;
    const bundle = await buildReplayMasterBundle(this, run, step, args, this.clock, replanContext);
    assertReplayBundleExecutable(bundle);
    const readyStep = patchReplayStep(step, {
    ...
    let readyRun = patchReplayRun(run, {
      status: "WAITING_GPT_MASTER",
    ...
    if (workItem) readyRun = patchReplayRun(readyRun, { current_work_item_id: workItem.work_item_id, automation_status: "waiting_gpt" }, tick);
    const timeline = replayTimelineEvent(readyRun, readyStep, {
    ...
    const result = { ok: true, backtest_id: readyRun.backtest_id, step_id: readyStep.step_id, status: readyRun.status, bundle_id: bundle.bundle_id, bundle: projectReplayBundle(bundle, { view: "compact" }), work_item: workItem ? deskWorkSummary(workItem) : null, next_action: nextReplayAction(readyRun.status) };
    const committed = await this.#commitReplayMutation({
      run,
      runPatch: readyRun,
  ...
  async saveReplayMasterAnalysis(args = {}) {
    ...
    const workItem = workItemId ? await this.#getDocument(COLLECTIONS.deskAgentWorkItems, workItemId).catch(() => null) : null;
    ...
    if (begin.existing_result) return begin.existing_result;
    const steps = selectBacktestSteps(await this.#listDocuments(COLLECTIONS.deskReplaySteps, 500).catch(() => []), run.backtest_id);
    const step = resolveReplayStep(steps, run, args.step_id, "MASTER");
    assertReplayContractSave(run, args, "master", step);
    const master = normalizeReplayMasterAnalysis(args, run, step, tick);
    const setupDocs = buildReplaySetupDocs(master, run, step, tick);
    const thesis = normalizeReplayActiveThesis(args.active_thesis || deriveActiveThesisFromMaster(master), master, run, step, tick);
    ...
    if (args.context_transmission) {
      const context = normalizeReplayContextTransmission(args.context_transmission, run, step, { linked_analysis_id: master.analysis_id }, tick);
      writes.push({ collection: COLLECTIONS.deskReplayContextTransmissions, documentId: context.context_id, data: context });
    }
    const savedStep = patchReplayStep(step, {
    ...
    const savedRun = patchReplayRun(run, {
    ...
    const timeline = replayTimelineEvent(savedRun, savedStep, {
    ...
    return run.automation_enabled ? { ...committed.result, automation: await this.driveReplayAutomation({ backtest_id: run.backtest_id }) } : committed.result;
  }

  async advanceReplayClock(args = {}) {
    const tick = this.clock.now();
    const run = await this.#getDocument(COLLECTIONS.deskReplayRuns, args.backtest_id);
    assertReplayCanAdvance(run, args);
    const steps = selectBacktestSteps(await this.#listDocuments(COLLECTIONS.deskReplaySteps, 500).catch(() => []), run.backtest_id);
    const sequence = steps.length + 1;
    const timestamp = offsetIso(run.current_replay_time, (Number(args.minutes) || 15) * 60 * 1000);
    if (run.end_time && Date.parse(timestamp) > Date.parse(run.end_time)) {
      ...
      if (begin.existing_result) return begin.existing_result;
      const ended = patchReplayRun(run, { status: "COMPLETED", completed_at_utc: tick.utc }, tick);
      const result = { ok: true, backtest_id: ended.backtest_id, status: ended.status, next_action: nextReplayAction(ended.status) };
      return (await this.#commitReplayMutation({ run, runPatch: ended, mutation: begin.mutation, result, tick })).result;
    }
    ...
    const advancedRun = patchReplayRun(run, {
      status: "MONITOR_DATA_PREPARING",
      current_step_id: step.step_id,
      current_replay_time: timestamp,
    }, tick);
    const timeline = replayTimelineEvent(advancedRun, step, {
    ...
    const result = { ok: true, backtest_id: advancedRun.backtest_id, step_id: step.step_id, status: advancedRun.status, current_replay_time: timestamp, step, next_action: nextReplayAction(advancedRun.status) };
    return (await this.#commitReplayMutation({
      run,
  ...
  async prepareReplayMonitorBundle(args = {}) {
    ...
    if (begin.existing_result) return begin.existing_result;
    const steps = selectBacktestSteps(await this.#listDocuments(COLLECTIONS.deskReplaySteps, 500).catch(() => []), run.backtest_id);
    const step = resolveReplayStep(steps, run, args.step_id, "MONITOR");
    const monitorStep = patchReplayStep(step, { status: "MONITOR_DATA_PREPARING", timestamp_paris: args.timestamp_paris || step.timestamp_paris }, tick);
    const monitorRun = patchReplayRun(run, { status: "MONITOR_DATA_PREPARING", current_step_id: monitorStep.step_id, current_replay_time: monitorStep.timestamp_paris }, tick);
    const replayDocs = {
      master: selectLatestReplayMaster(await this.#listDocuments(COLLECTIONS.deskReplayMasterAnalyses, 500).catch(() => []), run.backtest_id),
      thesis: selectReplayActiveThesis(await this.#listDocuments(COLLECTIONS.deskReplayActiveTheses, 500).catch(() => []), run.backtest_id),
      setups: selectReplayScopedSetups(await this.#listDocuments(COLLECTIONS.deskReplaySetups, 500).catch(() => []), run.backtest_id),
      monitors: selectReplayMonitors(await this.#listDocuments(COLLECTIONS.deskReplayMonitors, 500).catch(() => []), run.backtest_id),
      positions: selectReplayPositions(await this.#listDocuments(COLLECTIONS.deskReplayPositions, 500).catch(() => []), run.backtest_id),
    };
    const bundle = await buildReplayMonitorBundle(this, monitorRun, monitorStep, replayDocs, args, this.clock);
    assertReplayBundleExecutable(bundle);
    const readyStep = patchReplayStep(monitorStep, {
    ...
    let readyRun = patchReplayRun(monitorRun, { status: "WAITING_GPT_MONITOR", latest_bundle_id: bundle.bundle_id }, tick);
    const workItem = readyRun.automation_enabled ? buildReplayAgentWorkItem({ run: readyRun, step: readyStep, bundle, tick }) : null;
    if (workItem) readyRun = patchReplayRun(readyRun, { current_work_item_id: workItem.work_item_id, automation_status: "waiting_gpt" }, tick);
    const timeline = replayTimelineEvent(readyRun, readyStep, {
  ...
  async getReplayBundleManifest(args = {}) {
    const docs = await this.#listDocuments(COLLECTIONS.deskReplayBundles, 500).catch(() => []);
    return projectReplayBundle(selectReplayBundleForRead(docs, args), { ...args, view: "manifest" });
  }

  async getReplayBundleSection(args = {}) {
    const docs = await this.#listDocuments(COLLECTIONS.deskReplayBundles, 500).catch(() => []);
    return getReplayBundleSectionView(selectReplayBundleForRead(docs, args), args);
  }

  async getReplaySnapshot(args = {}) {
    const docs = await this.#listDocuments(COLLECTIONS.deskReplayBundles, 500).catch(() => []);
    return getReplaySnapshotView(selectReplayBundleForRead(docs, args), args);
  }
  ...
  async saveReplayMonitor(args = {}) {
    ...
    const steps = selectBacktestSteps(await this.#listDocuments(COLLECTIONS.deskReplaySteps, 500).catch(() => []), run.backtest_id);
    const step = resolveReplayStep(steps, run, args.step_id, "MONITOR");
    assertReplayContractSave(run, args, "monitor", step);
    const monitor = normalizeReplayMonitor(args, run, step, tick);
    ...
    if (args.monitor_context_transmission) {
      const context = normalizeReplayContextTransmission(args.monitor_context_transmission, run, step, { linked_monitor_id: monitor.monitor_id }, tick);
      writes.push({ collection: COLLECTIONS.deskReplayContextTransmissions, documentId: context.context_id, data: context });
    }
    const existingThesis = selectReplayActiveThesis(await this.#listDocuments(COLLECTIONS.deskReplayActiveTheses, 500).catch(() => []), run.backtest_id);
    if (existingThesis) {
      const updatedThesis = patchReplayThesis(existingThesis, monitor, tick);
      writes.push({ collection: COLLECTIONS.deskReplayActiveTheses, documentId: updatedThesis.thesis_id, data: updatedThesis });
    }
    const existingSetups = selectReplayScopedSetups(await this.#listDocuments(COLLECTIONS.deskReplaySetups, 500).catch(() => []), run.backtest_id);
    const monitorSetupDocs = buildReplaySetupDocsFromMonitor({
      ...
      existingSetups,
      tick,
      makeSetupId: (value) => scopedReplayChildId(run, "setup", value),
    });
    ...
    const result = { ok: true, backtest_id: savedRun.backtest_id, step_id: savedStep.step_id, status: savedRun.status, monitor_id: monitor.monitor_id, next_action: nextReplayAction(savedRun.status) };
    const committed = await this.#commitReplayMutation({
      run, runPatch: savedRun, mutation: begin.mutation, writes, result, tick,
      preconditions: args.work_item_id ? [{
        collection: COLLECTIONS.deskAgentWorkItems,
  ...
  async applyReplayMonitorResult(args = {}) {
    ...
    const steps = selectBacktestSteps(await this.#listDocuments(COLLECTIONS.deskReplaySteps, 500).catch(() => []), run.backtest_id);
    const step = resolveReplayStep(steps, run, args.step_id, "MONITOR");
    const monitor = selectReplayMonitorForStep(await this.#listDocuments(COLLECTIONS.deskReplayMonitors, 500).catch(() => []), run.backtest_id, step.step_id);
    if (!monitor) throw new Error("replay_monitor_not_found_for_step");
    const positions = selectReplayPositions(await this.#listDocuments(COLLECTIONS.deskReplayPositions, 500).catch(() => []), run.backtest_id);
    const apply = buildReplayMonitorApplication(run, step, monitor, positions, tick);
    const begin = await this.#beginReplayMutation(run, args, {
    ...
    const appliedStep = patchReplayStep(step, { status: "SIMULATION_UPDATED", applied_decision: apply.action }, tick);
    const appliedRun = patchReplayRun(run, {
      status: apply.run_status,
      latest_position_id: apply.position?.position_id || run.latest_position_id || null,
    }, tick);
    const timeline = replayTimelineEvent(appliedRun, appliedStep, {
    ...
    const result = { ok: true, backtest_id: appliedRun.backtest_id, step_id: appliedStep.step_id, status: appliedRun.status, action: apply.action, position: apply.position || null, next_action: nextReplayAction(appliedRun.status) };
    return (await this.#commitReplayMutation({ run, runPatch: appliedRun, mutation: begin.mutation, writes, result, tick })).result;
  }
  ...
  async simulateReplayInterval(args = {}) {
    ...
    const step = resolveReplayStep(steps, run, args.step_id, "MONITOR");
    const positions = selectReplayPositions(await this.#listDocuments(COLLECTIONS.deskReplayPositions, 500).catch(() => []), run.backtest_id);
    const setups = selectReplayScopedSetups(await this.#listDocuments(COLLECTIONS.deskReplaySetups, 500).catch(() => []), run.backtest_id);
    const position = positions[0] || null;
    const from = args.from_timestamp || step.previous_timestamp_paris || offsetIso(step.timestamp_paris, -15 * 60 * 1000);
    const to = args.to_timestamp || step.timestamp_paris || run.current_replay_time;
    const simulation = await buildReplayIntervalSimulation(this, run, step, position, setups, { from, to }, tick);
    const simulatedStep = patchReplayStep(step, {
      status: "MONITOR_DATA_PREPARING",
      simulation_ref: { collection: COLLECTIONS.deskReplayTradeSimulations, document_id: simulation.simulation_id },
      anti_lookahead_compliant: true,
      validation_result: "passed",
    }, tick);
    const simulatedRun = patchReplayRun(run, {
      status: "MONITOR_DATA_PREPARING",
    ...
    const timeline = replayTimelineEvent(simulatedRun, simulatedStep, {
    ...
    const result = { ok: true, backtest_id: simulatedRun.backtest_id, step_id: simulatedStep.step_id, status: simulatedRun.status, simulation, next_action: nextReplayAction(simulatedRun.status) };
    return (await this.#commitReplayMutation({
      run,
  ...
  async getReplayTimeline({ backtest_id, limit = 200 } = {}) {
    const timeline = selectReplayTimeline(await this.#listDocuments(COLLECTIONS.deskReplayTimeline, Math.max(50, Math.min(Number(limit) || 200, 500))).catch(() => []), backtest_id).slice(0, Math.max(1, Math.min(Number(limit) || 200, 500)));
  ...
  async #beginReplayMutation(run, args, config) {
    const idempotencyKey = String(args.idempotency_key || "").trim();
    if (idempotencyKey) {
      const id = replayIdempotencyDocumentId(run.backtest_id, idempotencyKey);
      const existing = await this.#getDocument(COLLECTIONS.deskReplayIdempotency, id).catch(() => null);
      if (existing) {
  ...
  async #commitReplayMutation({ run, runPatch, mutation, writes = [], result, tick, preconditions = [] }) {
    ...
    await this.#setDocument(COLLECTIONS.deskReplayRuns, run.backtest_id, nextRun, { merge: true });
    const id = replayIdempotencyDocumentId(run.backtest_id, mutation.idempotency_key);
    await this.#setDocument(COLLECTIONS.deskReplayIdempotency, id, {
      idempotency_id: id,
  ...
  async #readStorageObject(storagePath, generation) {
    ...
    return { buffer, text, metadata: { generation: generation == null ? null : String(generation), size_bytes: buffer.length } };
  }

  async #markPackBuildInvalid(pack, error) {
    if (!pack?.pack_build_id || !isBlockingIntegrityError(error)) return;
    const tick = this.clock.now();
    await this.#setDocument(COLLECTIONS.deskPackBuilds, pack.pack_build_id, {
...
function assertDataset(dataset) {
  if (!DATASETS.includes(dataset)) {
    throw new Error(`dataset_not_allowed:${dataset}`);
  }
}

function deskError(code, message, details = {}) {
...
function assertDatasetManifestScope(pack, ref, dataset) {
  ...
  if (mismatches.length) {
    throw deskError("DATASET_SCOPE_MISMATCH", "Dataset manifest entry does not match its pack build.", { dataset, mismatches });
  }
  return true;
}
...
function validatedDatasetResponse({ pack, ref, integrity, dataset, format, max_rows, as_of_utc }) {
  ...
  if (decisionCutoffUtc && sourceCutoffUtc && Date.parse(decisionCutoffUtc) > Date.parse(sourceCutoffUtc)) {
    throw deskError("REPLAY_SOURCE_COVERAGE_INSUFFICIENT", "Requested decision cutoff exceeds the immutable source pack coverage.", {
      pack_id: pack.pack_id,
      pack_build_id: pack.pack_build_id,
  ...
  if (format === "csv") {
    return { ...base, csv: serializeDatasetRows(analysis.columns, limitedRows) };
  }
...
function filterDatasetRowsAtCutoff(rows, dataset, cutoffUtc) {
  ...
  return (rows || []).filter((row) => {
    ...
    const timeframeMinutes = replayTimeframeMinutes(row?.timeframe);
    const observationMs = Date.parse(row?.timestamp_utc || row?.timestamp_paris || "");
    if (timeframeMinutes >= 60 && Number.isFinite(observationMs) && observationMs + timeframeMinutes * 60 * 1000 > cutoffMs) {
      return false;
    }
    return true;
  });
...
function replaySourceCoverage(pack) {
...
async function driveReplayAutomationLoop(store, { backtest_id, max_transitions = 8 } = {}) {
  const transitions = [];
  for (let index = 0; index < Math.max(1, Math.min(Number(max_transitions) || 8, 12)); index += 1) {
    const state = await store.getReplayState({ backtest_id });
    ...
    if (["MASTER_DATA_PREPARING", "CREATED", "REPLAN_REQUIRED"].includes(run.status)) {
      result = await store.prepareReplayMasterBundle({ ...common, idempotency_key: key("prepare-master"), include_raw_refs: false });
    } else if (["READY_FOR_NEXT_MONITOR", "WAITING_NEXT_STEP", "SIMULATION_UPDATED", "MASTER_MATERIALIZED"].includes(run.status)) {
      result = await store.advanceReplayClock({ ...common, idempotency_key: key("advance-clock"), minutes: replayCadenceMinutes(run, state) });
    } else if (run.status === "MONITOR_DATA_PREPARING" && !step?.simulation_ref) {
      result = await store.simulateReplayInterval({ ...common, idempotency_key: key("simulate-interval") });
    } else if (run.status === "MONITOR_DATA_PREPARING") {
      result = await store.prepareReplayMonitorBundle({ ...common, idempotency_key: key("prepare-monitor"), timestamp_paris: step?.timestamp_paris, include_raw_refs: false });
    } else if (["MONITOR_SAVED", "MONITOR_APPLIED"].includes(run.status)) {
      result = await store.applyReplayMonitorResult({ ...common, idempotency_key: key("apply-monitor") });
    } else {
      return { ok: false, status: "BLOCKED", backtest_id, replay_status: run.status, reason: "unsupported_automatic_transition", transitions };
    }
    transitions.push({ from_status: run.status, operation: result?.next_action || nextReplayAction(run.status), to_status: result?.status || null, revision: result?.revision ?? null });
  }
  const finalState = await store.getReplayState({ backtest_id });
...
async function buildMasterBundle(store, {
  date,
  session = "asia_open",
  pack_id,
  timezone = "Europe/Paris",
  ...
  const tick = clock.now();
  const pack = pack_id
    ? await store.getDeskPack({ pack_id, include_draft: false })
    : await store.getLatestAsiaOpenPack({ date, timezone }).then((summary) => store.getDeskPack({ pack_id: summary.pack_id }));
  const contracts = await store.getActiveContracts();
  const contract_context = contractContext(contracts, "master", { tick, pinnedForReplay: false });
  const macro_calendar = await store.getMacroCalendar({ pack_id: pack.pack_id, date: pack.date }).catch((error) => ({ warning: error.message, events: [] }));
  const news_digest = await store.getNewsDigest({ pack_id: pack.pack_id, date: pack.date, session: pack.session }).catch((error) => ({ warning: error.message, items: [] }));
  const latest_master_analysis = await store.getLatestMasterAnalysis({ session, before_date: pack.date }).catch(() => ({ analysis: null }));
  const active_thesis = await store.getActiveThesis({ session, date: pack.date, status: "any" }).catch(() => ({ active_thesis: null }));
  return {
    ok: true,
    bundle_id: `master_analysis_bundle_${pack.pack_id}`,
    contract: contracts.master_contract,
    contract_context,
    contract_handshake: contractHandshake("master_live", contract_context, {
      saveTool: "save_master_analysis",
    }),
    pack: compactPack(pack),
    datasets_summary: summarizeDatasetRefs(pack.datasets || {}, include_raw_refs),
    macro_calendar,
    news_digest,
    previous_context: active_thesis.active_thesis || null,
    latest_master_analysis: latest_master_analysis.analysis || null,
    latest_monitor: null,
    session_snapshot: null,
    level_map: null,
    technical_events: [],
    cross_asset_snapshot: null,
    data_quality: pack.quality || {},
    anti_lookahead_policy: {
      cutoff: pack.data_cutoff || null,
      rule: "Use macro actuals only when published_at_paris/actual_published_at_paris is <= pack cutoff; ignore hidden or post-cutoff actuals. Do not use candles after pack cutoff.",
      macro_actuals_visible_until_cutoff: true,
      macro_actuals_after_cutoff_hidden: true,
    },
    timezone,
...
async function buildMonitorBundle(store, args = {}, clock = new SystemClock()) {
  const _t = clock.now();
  const resolvedScope = resolveOperationalReadScope(args, { requireMaster: true, requireThesis: true });
  const checkpoint = args.timestamp_paris || resolvedScope.cutoff_paris;
  if (Date.parse(checkpoint) !== Date.parse(resolvedScope.as_of_utc)) {
    throw deskError("INVALID_SCOPE", "timestamp_paris and as_of_utc must identify the same instant.", {
      timestamp_paris: checkpoint,
      as_of_utc: resolvedScope.as_of_utc,
    });
  }
  ...
  const session = resolvedScope.session;
  const timezone = resolvedScope.timezone;
  const include_raw_refs = args.include_raw_refs !== false;
  const contracts = await safeRead(store.getActiveContracts(), {});
  const contract_context = contractContext(contracts, "monitor", { tick: _t, pinnedForReplay: false });
  const context = await resolveMonitorContext(store, args);
  const active_thesis = context.activeThesis;
  const latest_master_analysis = context.latestMaster;
  const latest_monitor = await getLatestOperationalMonitor(store, {
    ...operationalSelectorArgs(resolvedScope),
    thesis_id: active_thesis.thesis_id,
    master_id: latest_master_analysis.analysis_id,
    limit: 1,
  }).then((result) => result.latest_monitor);
  const features = await readFeatureContext(store, { date, session, activeThesis: active_thesis, latestMaster: latest_master_analysis, timestamp_paris: checkpoint });
  return {
    ok: true,
    bundle_id: `monitor_context_bundle_${active_thesis?.thesis_id || "none"}_${stableVNextId("ts", checkpoint, session, _t)}`,
    contract_context,
    contract_handshake: contractHandshake("manual_m15_monitor", contract_context, {
      saveTool: "save_hourly_monitor",
    }),
    strategy_id: resolvedScope.strategy_id,
    trading_date: resolvedScope.trading_date,
    run_id: resolvedScope.run_id,
    ...
    rolling_1h_snapshot: null,
    rolling_4h_snapshot: null,
    ...
    data_quality: {
      status: active_thesis || latest_master_analysis ? "ready" : "degraded",
      warnings: [
        ...(active_thesis ? [] : ["active_thesis_not_found_using_latest_master_fallback"]),
        ...(latest_master_analysis ? [] : ["latest_master_analysis_not_found"]),
        ...(features.cross?.stale_check?.is_stale ? ["cross_asset_delta_stale_or_missing"] : []),
      ],
      include_raw_refs,
    },
    timezone,
...
function summarizeDatasetRefs(datasets, includeRawRefs) {
  return Object.fromEntries(Object.entries(datasets).map(([name, ref]) => [name, {
    row_count: ref.row_count ?? null,
    format: ref.format || null,
    from_time: ref.from_time || null,
    to_time: ref.to_time || null,
    timezone: ref.timezone || null,
    storage_path: includeRawRefs ? ref.storage_path || null : undefined,
    checksum: ref.checksum || null,
  }]));
}

function datasetRef(pack, dataset) {
  const datasets = pack.datasets || {};
  if (datasets[dataset]) {
    return datasets[dataset];
  }
  if (dataset === "ny_close_mega_caps" && datasets.mega_caps_premarket) {
    return datasets.mega_caps_premarket;
  }
  if (dataset === "mega_caps_premarket" && datasets.ny_close_mega_caps) {
    return datasets.ny_close_mega_caps;
  }
  return null;
}
...
function parseDatasetText(text, ref, { maxRows }) {
  ...
  if (format.endsWith(".json") || format.includes("json")) {
    ...
    if (Array.isArray(parsed.items)) {
      return parsed.items.slice(0, maxRows);
    }
    if (Array.isArray(parsed.events)) {
      return parsed.events.slice(0, maxRows);
    }
    return parsed;
  }
...
function normalizeMacroDataset(text, ref, { date, importance_min }) {
  ...
  const events = (Array.isArray(rows) ? rows : rows.events || []).filter((event) => impactRank(event.impact) >= impactRank(importance_min));
  return { date, importance_min, events };
}

function resolvePackCutoffUtc(pack) {
...
function impactRank(value) {
  ...
  if (["high", "red", "critical"].includes(text)) {
    return 3;
  }
  if (["medium", "orange"].includes(text)) {
    return 2;
  }
  if (["low", "yellow"].includes(text)) {
    return 1;
  }
  return 0;
}

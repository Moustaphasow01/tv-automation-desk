...
export class DeskStrategyAuditService {
  constructor({ persistence, clock, host, market, port }) {
  ...
  async getAuditState(args = {}) {
    ...
    return this.port.buildAuditState(this.host, args, {
  ...
  async getNyOpenStrategyState(args = {}) {
    return this.port.buildNyOpenStrategyState(this.host, {
      ...(args || {}),
  ...
  async prepareNyOpenMasterBundle(args = {}) {
    ...
    const cutoff_paris = args.cutoff_paris || this.port.nyOpenCutoffParis(date);
    const result = await this.host.prepareMasterCutoffBundleJob({
      ...args,
      ...this.port.nyOpenOperationalScope(date, cutoff_paris),
      cutoff_paris,
      instruments: ["MNQ", "MES", "NQ", "ES"],
      save: args.save !== false,
    });
    return this.port.normalizeNyOpenBundlePrep(result, { date, cutoff_paris });
  }

  async getStrategyPerformance(args = {}) {
    const strategy_id = this.port.resolveStrategySelection(args);
    ...
    const trades = this.port.filterStrategyTrades(tradeDocs, args, filterId);
    const setups = this.port.filterStrategySetups(setupDocs, args, filterId);
    return this.port.buildStrategyPerformance({ strategy_id, trades, setups, args });
  }

  async getStrategyCalendar(args = {}) {
    const strategy_id = this.port.requireStrategyId(args);
    const docs = await this.loadStrategyDocuments(strategy_id);
    return this.port.buildStrategyCalendar(docs, args, this.clock);
  }

  async getStrategyDayDetail(args = {}) {
    const strategy_id = this.port.requireStrategyId(args);
    const docs = await this.loadStrategyDocuments(strategy_id);
    return this.port.buildStrategyDayDetail(docs, {
      ...args,
      strategy_id,
  ...
  async getLiveTimelineEventDetail(args = {}) {
    const strategy_id = this.port.requireStrategyId(args);
    const docs = await this.loadStrategyDocuments(strategy_id);
    return this.port.buildLiveTimelineEventDetail(docs, {
      ...args,
      strategy_id,
      as_of_paris: this.clock.now().paris,
    });
  ...
  async recomputeStrategyPerformance(args = {}) {
    ...
    const strict_replay = await this.applyNyOpenStrictReplay({ ...args, strategy_id })
      .catch((error) => ({ ok: false, skipped: true, error: this.port.publicReplayError(error) }));
    const docs = await this.loadStrategyDocuments(strategy_id);
    const recomputed = this.port.recomputeStrategyPerformanceDocs(docs, { ...args, strategy_id }, tick);
  ...
  async markNyOpenStrategyEvent(args = {}) {
    const tick = this.clock.now();
    const action = this.port.normalizeNyOpenAction(args);
    ...
    const patch = this.port.strategyActionPatch(action, tick);
    ...
    const audit = this.port.strategyAuditLog({
  ...
  async applyNyOpenStrictReplay(args = {}) {
    ...
    const sourceSetups = Array.isArray(args.setups) && args.setups.length
      ? args.setups
      : this.port.filterStrategySetups(
        await this.persistence.listDocuments(COLLECTIONS.deskSetups, 500).catch(() => []),
        this.port.strictReplaySetupFilters(args),
        strategy_id,
      );
    const setup = this.port.selectNyOpenStrictSetup(sourceSetups, args);
    if (!setup) {
      return { ok: true, skipped: true, reason: "ny_open_strict_setup_not_found", strategy_id };
    }
    const replayWindow = this.port.nyOpenStrictReplayWindow(setup, args);
    const modeResults = [];
    for (const pricing_mode of this.port.nyOpenStrictPricingModes()) {
      const replay = await this.market.replaySetup(this.port.strictSetupForReplay(setup, pricing_mode), {
        ...args,
        pricing_mode,
        replay_id: this.port.nyOpenStrictReplayId(setup, pricing_mode, tick),
      ...
      await this.persistence.setDocument(
        ...
        replay.replay_id || `${this.port.setupDocumentId(setup)}_${pricing_mode}_strict_latest`,
        replay,
        { merge: true },
      );
      const trade = this.port.nyOpenStrictTradeFromReplay({ setup, replay, replayWindow, strategy_id, tick, pricing_mode });
      if (trade) {
    ...
    const setupPatch = this.port.nyOpenStrictSetupPatch({ setup, modeResults, replayWindow, strategy_id, tick, pricing_mode: args.pricing_mode });
    await this.persistence.setDocument(COLLECTIONS.deskSetups, this.port.setupDocumentId(setup), setupPatch, { merge: true });
    const selected = this.port.selectedStrictModeResult(modeResults, args.pricing_mode);
    const audit = this.port.strategyAuditLog({
      strategy_id,
      action: "nyopen_strict_replay",
      document_type: selected.trade ? "trade" : "setup",
      document_id: selected.trade?.trade_id || this.port.setupDocumentId(setup),
    ...
    await this.persistence.setDocument(COLLECTIONS.deskAuditLogs, audit.audit_id, audit, { merge: true });
    return this.port.summarizeNyOpenStrictReplay({ setup, modeResults, audit, replayWindow, strategy_id, pricing_mode: args.pricing_mode });
  }
  ...
  async loadStrategyDocuments(strategy_id) {
    ...
    return this.port.selectStrategyDocuments({
      strategy_id,
      masters,
      theses,
      setups,
  ...
  async resolveStrategyActionTarget(args, action) {
    ...
    if (args.trade_id) {
      ...
      const trade = trades.find((item) => item.trade_id === args.trade_id && this.port.strategyDocMatches(item, strategy_id));
    ...
    const setup = setups.find((item) => this.port.setupDocumentId(item) === setupId || item.setup_id === setupId || item.setup_record_id === setupId);
    if (!setup) return null;
    if (action === "mark_setup_triggered") return { type: "setup", id: this.port.setupDocumentId(setup), doc: setup };
    return { type: "setup", id: this.port.setupDocumentId(setup), doc: setup };
  }

...
test("causal grains context is adjudicated at the bus without mutating the raw signal", () => {
  ...
  assert.equal(
    evaluateCausalGrainContextAtBus({
...
test("causal grains context rejects temporal, quality and multiplier contract defects", () => {
  for (const [edit, reason] of [
    [
      (signal) => {
        signal.source_data_cutoff_utc = "2026-09-04T15:05Z";
      },
      "GRAIN_CONTEXT_AFTER_DECISION_TIME",
    ],
    [
      (signal) => {
        signal.expires_at_utc = NOW;
      },
      "GRAIN_CONTEXT_SIGNAL_EXPIRED",
    ],
    [
      (signal) => {
        signal.setup.context.source_data_cutoff_utc = "2026-09-04T14:55Z";
      },
      "GRAIN_CONTEXT_CUTOFF_MISMATCH",
    ],
    [
      (signal) => {
        signal.setup.context.data_quality = { tradeable: false };
      },
      "GRAIN_CONTEXT_SOURCE_DATA_BLOCKED",
    ],
    [
      (signal) => {
        delete signal.setup.context.data_quality;
      },
      "GRAIN_CONTEXT_SOURCE_DATA_BLOCKED",
    ],
    [
      (signal) => {
        signal.setup.context.valid_until_utc = NOW;
      },
      "GRAIN_CONTEXT_EXPIRED_OR_UNAVAILABLE",
    ],
    [
      (signal) => {
        signal.setup.context.instrument = "ZC";
      },
      "GRAIN_CONTEXT_IDENTITY_MISMATCH",
    ],
    [
      (signal) => {
        delete signal.setup.context.macro_event_risk;
      },
      "GRAIN_CONTEXT_CALENDAR_UNAVAILABLE",
    ],
    [
      (signal) => {
        signal.setup.context.macro_event_risk.events = [{}];
      },
      "GRAIN_CONTEXT_CALENDAR_INVALID",
    ],
    [
      (signal) => {
        delete signal.setup.context.macro_event_risk.coverage;
      },
      "AGRI_CALENDAR_KNOWLEDGE_UNPROVEN",
    ],
    [
      (signal) => {
        signal.setup.context.macro_event_risk.coverage = { admissible: true };
      },
      "AGRI_CALENDAR_KNOWLEDGE_UNPROVEN",
    ],
    [
      (signal) => {
        signal.setup.context.risk_multiplier = 5;
      },
      "GRAIN_CONTEXT_RISK_MULTIPLIER_INVALID",
    ],
  ]) {
    const signal = rawSignal();
    edit(signal);
...
test("known upcoming agri event waits without preventing raw publication", () => {
  ...
  signal.setup.context.macro_event_risk.events = [
    {
      event_timestamp_utc: "2026-09-04T15:15Z",
      source_published_at_utc: "2026-09-01T12:00Z",
    },
  ];
  const result = evaluateCausalGrainContextAtBus({ signal, nowUtc: NOW });
  assert.equal(result.decision, "WAIT");
...
function rawSignal() {
  return {
    ...
    expires_at_utc: "2026-09-04T15:45Z",
    signal_quality: {

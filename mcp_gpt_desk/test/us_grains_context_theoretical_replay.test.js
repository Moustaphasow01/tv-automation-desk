...
test("contextual replay preserves raw/context detection while consuming the causal adapter for one shared entry and exit", () => {
  ...
  const withOutcome = replayUsGrainsContextTheoretical({
    ...
    rowsBySymbol: {
      ...input.rowsBySymbol,
      [`${signal.instrument}1!:1`]: [...input.rowsBySymbol[`${signal.instrument}1!:1`], ...executionRows],
    },
  ...
  assert.equal(withOutcome.context.simulated_signal_count, suite.context_accepted_count);
  ...
  assert.deepEqual(withFutureExecution.context_decisions, withOutcome.context_decisions);
...
test("contextual replay reports no closed R as null, never as a tradable zero", () => {
  ...
  assert.equal(report.closed_r.interpretation, "THEORETICAL_CLOSED_OUTCOMES_NOT_TRADABLE");
...
function outcomeFor(report, signalId) {
  return report.theoretical_outcomes.find((outcome) => outcome.signal_id === signalId);
}
...
function outcomeRows(signal) {
  ...
  return [
    ...
    isLong
      ...
      : m1(plusMinutes(signal.generated_at_utc, 1), entry, entry, target - 0.25),
  ];
}

function causalInput(extra = {}) {
  const prior = m5Day("2026-07-06T13:30:00.000Z", Array.from({ length: 58 }, () => 99));
  const current = m5Day("2026-07-07T13:30:00.000Z", Array.from({ length: 20 }, (_, index) => 100 + index * 0.5));
  return {
    instruments: ["ZW"],
    rowsBySymbol: { "ZW1!:5": [...prior, ...current], "ZW1!:1": warmupM1Prefix() },
    ...extra,
  };
}
...
function m5Day(startUtc, prices) {
  return prices.map((price, index) => m1(plusMinutes(startUtc, index * 5), price, price + 0.75, price - 0.25));
}

function m1(timestampUtc, open, high, low) {
  return { timestamp_utc: timestampUtc, timeframe: "1", is_closed: true, open, high, low, close: open, volume: 10 };
}

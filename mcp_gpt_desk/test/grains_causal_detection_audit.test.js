...
import {
  ...
  candlesToRowsBySymbol,
  compareSignals,
} from "../src/grains-causal-detection-audit.js";

const candle = (timestamp_utc, close = 1) => ({
  feed_id: "prod__tradingview__ZC1!__5",
  timestamp_utc,
  open: close,
  high: close,
  low: close,
...
const signal = (at, direction = "LONG") => ({
  signal_id: `s-${at}`,
  instrument: "ZC",
  direction,
  generated_at_utc: at,
  source_data_cutoff_utc: at,
  expires_at_utc: "2026-09-04T17:00:00.000Z",
  setup: { family: "VWAP_PULLBACK" },
  proposed_trade_plan: { plan_hash: "p1" },
  signal_quality: {
    strategy_suite_version: "us_grains_strategy_suite_causal_v2",
...
test("converts feed ids and checks causal prefix invariance", () => {
  ...
  const source = {
    asOf: at,
    candles: [
      candle("2026-09-04T14:55:00.000Z"),
      candle("2026-09-04T15:55:00.000Z", 9),
    ],
    signals: [signal(at)],
  };
  const detect = (input) => ({
    suite_version: "fixture",
    raw_signals: input.rowsBySymbol["ZC1!:5"].some(
      (row) => row.timestamp_utc === "2026-09-04T14:55:00.000Z",
    )
      ? [signal(at)]
      : [],
  });
...
test("reports changed prefix and signal fingerprint differences", () => {
  const at = "2026-09-04T15:00:00.000Z";
  const source = {
    candles: [candle("2026-09-04T14:55:00.000Z")],
    signals: [signal(at)],
  };
  const detect = (input) => ({
    raw_signals: input.asOfUtc === at ? [signal(at, "SHORT")] : [signal(at)],
  });
  const report = auditCausalDetection({
    source,
    detect,
  ...
  assert.equal(
    report.invariance.counterexamples[0].reason,
...
test("future OHLC changes do not enter a closed prefix", () => {
  ...
  const source = {
    candles: [candle("2026-09-04T14:55:00.000Z"), candle(future, 2)],
    signals: [signal(cutoff)],
  };
  const detect = (input) => ({
    raw_signals: input.rowsBySymbol["ZC1!:5"].some(
      (row) => row.timestamp_utc === "2026-09-04T14:55:00.000Z",
  ...
  const first = auditCausalDetection({
    source,
    detect,
    startDate: "2026-09-04",
    endDate: "2026-09-04",
  ...
  const altered = {
    ...source,
    candles: [source.candles[0], { ...source.candles[1], close: 999 }],
  };
  const second = auditCausalDetection({
    source: altered,
    detect,
    startDate: "2026-09-04",
    endDate: "2026-09-04",
...
test("audit detects a prefix signal entirely suppressed by future data in the full batch", () => {
  const at = "2026-09-04T15:00:00.000Z";
  const source = {
    candles: [candle("2026-09-04T14:55Z"), candle("2026-09-04T15:55Z")],
  };
  const detect = (input) => ({
    raw_signals: input.rowsBySymbol["ZC1!:5"].length === 1 ? [signal(at)] : [],
  });
  const report = auditCausalDetection({
    source,
    detect,
...
test("audit perturbs future prices and catches a detector that ignores its clock", () => {
  ...
  const detect = (input) => ({
    raw_signals: [
      signal(
        at,
        input.rowsBySymbol["ZC1!:5"].at(-1).close > 100 ? "SHORT" : "LONG",
      ),
    ],
  });
  const report = auditCausalDetection({
  ...
  assert.ok(
    report.invariance.counterexamples.some(
      (item) => item.reason === "FUTURE_PRICE_PERTURBATION_CHANGED_SIGNAL",
    ),
  );
});
...
test("legacy ledger unwraps the real signal.emitted envelope before matching family and source-open", () => {
  ...
  const row = {
    ...
    instrument: "ZC",
    direction: "LONG",
    generated_at_utc: old.generated_at_utc,
    proposed_trade_plan: old.proposed_trade_plan,
    payload: { type: "signal.emitted", payload: old },
  };
...
test("audit preserves coverage certificates and tests knowledge at each prefix", () => {
  const early = { asOf: "2026-09-01T00:00:00Z", datasetVersion: "known" };
  const late = { asOf: "2026-09-05T00:00:00Z", datasetVersion: "future" };
  const inputs = [];
  auditCausalDetection({
    source: {
      candles: [candle("2026-09-04T14:55:00Z")],
      agriCalendarCoverage: [early, late],
    },
    detect: (input) => {
      inputs.push(input);
      return { raw_signals: [] };
  ...
  assert.deepEqual(inputs[0].agriCalendarCoverage, [early, late]);
  assert.deepEqual(inputs[1].agriCalendarCoverage, [early]);
  assert.deepEqual(inputs[2].agriCalendarCoverage, [early, late]);
});

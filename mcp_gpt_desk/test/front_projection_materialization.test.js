...
test("PersistentDeskStore materializes Master and Monitor front projections with monotonic current state", async () => {
  ...
  const master = await store.saveMasterAnalysis({
    ...scope,
    analysis_id: "master-1",
    active_thesis_id: "thesis-1",
    created_at_paris: "2026-07-14T08:15:00+02:00",
    full_analysis: {},
  ...
  const monitor = await store.saveHourlyMonitor({
    ...scope,
  ...
  const retry = await store.saveHourlyMonitor({
    ...scope,
    monitor_id: "monitor-2",
    linked_master_analysis_id: "master-1",
    linked_active_thesis_id: "thesis-1",
    timestamp_paris: "2026-07-14T08:15:00+02:00",
    monitor_decision: { action: "retry" },
    thesis_health_score: {},
    front_projection: projection({ sourceType: "MONITOR", sourceId: "monitor-2", monitorId: "monitor-2", sequence: 2, revision: 2 }),
  });
  assert.equal(retry.front_projection.status, "idempotent");
  assert.equal(persistence.count(DESK_COLLECTIONS.deskFrontSnapshots), 2);
});
...
test("stale or invalid projections are recorded without replacing the canonical current state", async () => {
  ...
  const stale = await store.saveHourlyMonitor({
    ...scope,
    monitor_id: "monitor-stale",
    linked_master_analysis_id: "master-1",
    linked_active_thesis_id: "thesis-1",
    timestamp_paris: "2026-07-14T08:15:00+02:00",
    monitor_decision: {},
  ...
  const savedCanonicalMonitor = persistence.peek(DESK_COLLECTIONS.deskHourlyMonitors, "monitor-stale");

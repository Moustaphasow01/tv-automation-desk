    ...
    <div className="ep-page" data-testid="execution-providers-golden-master">
      <header className="ep-header">
        <div className="ep-header__title">
          ...
          <p>Connectivité des fournisseurs et cycle de vie des ordres</p>
        </div>
      ...
      <div className="ep-workspace">
        {(command || commandError) && (
          <div className="ep-panel"><div className="ep-panel__body">
            <strong>{command ? `Commande ${operatorCode(command.status)}` : "Commande refusée"}</strong>
            <p className="ep-empty" title={command?.commandId ?? commandError ?? undefined}>{command ? "La demande est suivie par le desk." : operatorCopy(commandError)}</p>
          </div></div>
        )}
        ...
        <section className="ep-kpi-strip" aria-label="Indicateurs Exécution & Fournisseurs">
          ...
          <KpiCell label="Fournisseurs actifs" value={String(data.summary.activeProviders)} detail={`${data.summary.degradedProviders} dégradés`} />
          <KpiCell label="Latence moy." value={`${data.summary.avgLatencyMs} ms`} />
          <KpiCell label="Taux d'exécution" value={formatPercent(data.summary.fillRatePct)} detail={`${formatSignedR(data.summary.slippageR)} de glissement`} />
              ...
              {data.providers.map((provider) => (
                <div key={provider.providerId} className="ep-provider-card">
                  ...
                  <div className="ep-provider-grid">
                    <div><small>Rôle</small><strong>{operatorCode(provider.role)}</strong></div>
                    <div><small>Connexion</small><strong>{operatorCopy(provider.adapter)}</strong></div>
                    <div><small>Latence</small><strong>{provider.latencyMs} ms</strong></div>
                    <div><small>Exécution</small><strong>{formatPercent(provider.fillRatePct)}</strong></div>
                    <div><small>Connectivité</small><strong>{operatorCode(provider.connectivity)}</strong></div>
                    <div><small>Dernier signal de vie</small><strong>{formatTime(provider.heartbeatAt)}</strong></div>
                  </div>
                </div>
              ))}
            ...
            <div className="ep-panel__body" style={{ padding: 0 }}>
              <div className="ep-table-scroll">
                <table className="ep-table">
                  <thead><tr><th>Compte</th><th>Fournisseur</th><th>Mode</th><th>Valeur du compte</th><th>Positions</th><th>État</th></tr></thead>
                  <tbody>
                    {data.accounts.map((account: ProviderAccount) => (
                      <tr key={account.accountId}>
                        <td><strong>{account.label}</strong></td>
                        <td>{providerShort(account.providerId)}</td>
                        <td>{operatorCode(account.mode)}</td>
        ...
        <div className="ep-row3">
          <section className="ep-panel" aria-label="Modes d'exécution">
            ...
            <div className="ep-panel__body">
              ...
              <div className="ep-mode-row"><span>Validation opérateur requise</span><StatusBadge tone={data.executionModes.entryOperatorApprovalRequired ? "warning" : "success"}>{data.executionModes.entryOperatorApprovalRequired ? "Oui" : "Non"}</StatusBadge></div>
              ...
              <div className="ep-mode-row"><span>Compte réel autorisé</span><StatusBadge tone={data.executionModes.liveAccountAllowed ? "danger" : "success"}>{data.executionModes.liveAccountAllowed ? "Oui" : "Non"}</StatusBadge></div>
            </div>
          </section>
        ...
        <section className="ep-panel" aria-label="Commandes fournisseur">
          <header>
          ...
          <div className="ep-panel__body" style={{ padding: 0 }}>
            <div className="ep-table-scroll">
              <table className="ep-table">
                <thead><tr><th>Heure</th><th>Ordre proposé</th><th>Instrument</th><th>Sens</th><th>Quantité</th><th>Type</th><th>Statut</th></tr></thead>
                <tbody>
                  {visibleCommands.map((item) => (
                    <tr key={item.commandId}>
                      <td>{formatTime(item.at)}</td>
                      <td title={item.orderIntentId}>{item.instrument} · {operatorCode(item.side)} · {formatTime(item.at)}</td>
                      <td>{item.instrument}</td>
                      <td><StatusBadge tone={item.side === "BUY" ? "success" : "danger"}>{operatorCode(item.side)}</StatusBadge></td>
                      <td>{item.quantity}</td>
                      <td>{operatorCode(item.commandType)}</td>
                      <td><StatusBadge tone={commandStatusTone(item.status)}>{operatorCode(item.status)}</StatusBadge></td>
                    </tr>
                  ))}
                  {!visibleCommands.length ? <tr><td colSpan={7}><p className="ep-empty">Aucune commande fournisseur publiée pour cet onglet.</p></td></tr> : null}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div className="ep-row4">
          <section className="ep-panel" aria-label="Exécutions récentes">
            <header><h2>Exécutions</h2><small>{data.fills.length}</small></header>
            <div className="ep-panel__body" style={{ padding: 0 }}>
              <table className="ep-table">
                ...
                <tbody>
                  {data.fills.map((event) => (
                    <tr key={event.eventId}>
                      ...
                      <td>{event.side ? <StatusBadge tone={event.side === "BUY" ? "success" : "danger"}>{operatorCode(event.side)}</StatusBadge> : "—"}</td>
                      <td>{event.fillQuantity ?? event.quantity ?? "—"}</td>
                      <td>{event.fillPrice != null ? event.fillPrice.toFixed(2) : "—"}</td>
                    </tr>
                  ))}
                  {!data.fills.length ? <tr><td colSpan={4}><p className="ep-empty">Aucune exécution publiée.</p></td></tr> : null}
                </tbody>
              </table>
            </div>
          </section>

          <section className="ep-panel" aria-label="Exécutions partielles">
            <header><h2>Exécutions partielles</h2><small>{data.partialFills.length}</small></header>
            <div className="ep-panel__body" style={{ padding: 0 }}>
              <table className="ep-table">
                ...
                <tbody>
                  ...
                  {!data.partialFills.length ? <tr><td colSpan={4}><p className="ep-empty">Aucune exécution partielle publiée.</p></td></tr> : null}
                </tbody>
              </table>
            </div>
          </section>
              ...
              <table className="ep-table">
                ...
                <tbody>
                  {data.rejectsAndCancels.map((event) => (
                    <tr key={event.eventId}>
                      <td>{formatTime(event.at)}</td>
                      <td>{operatorCode(event.eventType)}</td>
                      <td><StatusBadge tone="danger">{operatorCode(event.status)}</StatusBadge></td>
                    </tr>
                  ))}
        ...
        <div className="ep-row2">
                  ...
                  <tbody>
                    {data.incidents.map((incident) => (
                      <tr key={incident.incidentId}>
                        ...
                        <td><StatusBadge tone={incident.severity === "HIGH" ? "danger" : incident.severity === "MEDIUM" ? "warning" : "accent"}>{operatorCode(incident.severity)}</StatusBadge></td>
                      </tr>
                    ))}
          ...
          <section className="ep-panel" aria-label="Événements fournisseur">
                  ...
                  <tbody>
                    {data.events.map((event) => (
                      <tr key={event.eventId}>
                        <td>{formatTime(event.at)}</td>
                        <td>{operatorCode(event.eventType)}</td>
                        <td><Link to={event.route}>{operatorCopy(event.title)}</Link></td>
                        <td><StatusBadge tone={event.status === "RECEIVED" ? "success" : event.status === "STALE" ? "warning" : event.status === "BLOCKED" ? "danger" : "accent"}>{operatorCode(event.status)}</StatusBadge></td>
                      </tr>
                    ))}
...
function providerShort(providerId: string) {
  if (!providerId || ["unavailable", "undefined", "unknown", "none"].includes(providerId.trim().toLowerCase())) return "Non publié";

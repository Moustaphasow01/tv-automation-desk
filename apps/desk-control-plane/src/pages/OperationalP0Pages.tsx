  ...
  return (
    <div className="operator-page sessions-page">
      ...
      <section className="operator-kpi-strip" aria-label="Indicateurs sessions">
        ...
        <KpiCard label="DATE" value={shortDate(data.summary.tradingDate)} delta="trading date" />
      ...
      <section className="operator-grid operator-grid--top" aria-label="Sessions disponibles">
        <Card title="Registre des sessions" eyebrow="LIVE SCOPE" density="compact">
        ...
        <Card title="Lecture opérateur" eyebrow="ÉTAT COURANT" density="compact">
          <div className="operational-detail-list">
            {data.sessions.map((session) => <article key={session.sessionId}><div><strong>{session.label}</strong><small>{session.decision} · dernier monitor {session.lastMonitorAt}</small></div><StatusBadge tone={tone(session.status)}>{session.status}</StatusBadge></article>)}
          </div>
        </Card>
        <Card title="Accès contextuels" eyebrow="DRILL-DOWN" density="compact">
      ...
      <section className="operator-grid operator-grid--bottom" aria-label="Contrat et continuité des sessions">
        ...
        <Card title="Continuité" eyebrow="POINT-IN-TIME" density="compact"><div className="reconciliation-grid"><MetricBox label="Trading date" value={data.summary.tradingDate} /><MetricBox label="Sessions" value={data.summary.total} /><MetricBox label="Thèses actives" value={data.summary.activeTheses} /></div></Card>
        <Card title="Vérité des données" eyebrow="NO MOCK" density="compact"><p>{meta.warnings?.length ? meta.warnings.join(" · ") : "Aucun fallback local : toutes les lignes proviennent du BFF."}</p></Card>
      </section>
    </div>
  );
}
...
export function LivePlanPage() {
  ...
  return (
    <div className="operator-page live-plan-page">
      <ViewTruthBanner meta={meta} />
      <OperatorPageHeader title="Plan & Setup" description={`${data.scope.tradingDate} · ${data.scope.sessionId} · plan déterministe courant.`} actions={<><Link to="/live/timeline">Voir la timeline</Link><Link className="operator-primary-action" to="/live">Retour Live</Link></>} />
      <section className="operator-kpi-strip" aria-label="Indicateurs plan live">
        <KpiCard label="SESSION" value={data.summary.sessionStatus} delta={data.scope.mode} tone={tone(data.summary.sessionStatus)} />
        ...
        <KpiCard label="THÈSE" value={data.summary.thesisAvailable ? data.thesis.status : "ABSENTE"} delta={`${data.thesis.confidence}% confiance`} tone={data.summary.thesisAvailable ? "info" : "warning"} />
        <KpiCard label="SETUP" value={data.setup.status} delta={data.setup.instrument} tone={data.summary.setupAvailable ? "info" : "neutral"} />
        <KpiCard label="POSITION" value={data.position.status} delta={data.position.executionMode} tone={data.position.active ? "success" : "neutral"} />
...
export function LiveNewsPage() {
  ...
  if (query.isLoading) return <OperationalLoading title="Agenda & News" />;
  if (query.isError || !query.data) return <OperationalError title="Agenda & News" error={query.error} />;
  const { data, meta } = query.data;
  const nextEvent = data.macroEvents.find((event) => event.isNext) ?? null;
  return (
    <div className="operator-page live-news-page">
      <ViewTruthBanner meta={meta} />
      <OperatorPageHeader title="Agenda & News" description={`${data.scope.tradingDate} · fenêtre point-in-time · actuals et sources réelles.`} actions={<Link className="operator-primary-action" to="/live">Retour Live</Link>} />
      <section className="operator-kpi-strip" aria-label="Indicateurs agenda et news"><KpiCard label="MACRO" value={`${data.summary.macroEvents}`} delta="événements fenêtre" /><KpiCard label="IMPACT FORT" value={`${data.summary.highImpactEvents}`} delta="événements high" tone="warning" /><KpiCard label="HEADLINES" value={`${data.summary.headlines}`} delta="actualités disponibles" /><KpiCard label="SOURCES" value={`${data.summary.providers}`} delta="providers distincts" tone="info" /><KpiCard label="PROCHAIN" value={data.summary.nextMacroAt} delta={nextEvent?.title ?? "événement non identifié"} /><KpiCard label="PROXIMITÉ" value={data.summary.nearEvent ? "IMMINENT" : "NORMAL"} delta="fenêtre macro" tone={data.summary.nearEvent ? "warning" : "success"} /></section>
      <section className="operator-grid operator-grid--top" aria-label="Agenda macro">
        <Card title="Calendrier macro" eyebrow="ACTUAL / FORECAST / PREVIOUS" density="compact"><DataTable columns={macroColumns} rows={data.macroEvents.slice(0, 6)} rowKey={(row) => row.eventId} caption="Événements macro" emptyLabel="Aucun événement macro publié." /></Card>
        ...
        <Card title="Scope de lecture" eyebrow="ANTI-LOOKAHEAD" density="compact"><div className="reconciliation-grid"><MetricBox label="Session" value={data.scope.sessionId} /><MetricBox label="Date" value={data.scope.tradingDate} /><MetricBox label="Mode" value={data.scope.mode} /><MetricBox label="Disponibilité" value={meta.availability ?? "AVAILABLE"} /></div></Card>
      </section>
      <section className="operator-grid operator-grid--bottom" aria-label="Flux actualités">
        <Card title="Dernières actualités" eyebrow="NEWS FEED" density="compact"><DataTable columns={headlineColumns} rows={data.headlines.slice(0, 6)} rowKey={(row) => row.headlineId} caption="Actualités" emptyLabel="Aucune actualité publiée dans la fenêtre." /></Card>
        <Card title="Actifs couverts" eyebrow="CROSS-ASSET" density="compact"><div className="tag-cloud">{unique(data.headlines.flatMap((headline) => headline.assets)).slice(0, 18).map((asset) => <StatusBadge key={asset} tone="info">{asset}</StatusBadge>)}</div></Card>
...
export function LiveTimelinePage() {
  ...
  return (
    <div className="operator-page live-timeline-page">
      ...
      <section className="operator-kpi-strip" aria-label="Indicateurs timeline"><KpiCard label="ÉVÉNEMENTS" value={`${data.summary.total}`} delta="cycle courant" /><KpiCard label="EXÉCUTÉS" value={`${data.summary.completed}`} delta="matérialisés" tone="success" /><KpiCard label="EN ATTENTE" value={`${data.summary.waiting}`} delta="planifiés ou ready" tone="info" /><KpiCard label="EN RETARD" value={`${data.summary.delayed}`} delta="late, failed ou blocked" tone={data.summary.delayed ? "warning" : "neutral"} /><KpiCard label="DERNIER" value={data.summary.lastCompletedAt} delta="checkpoint clôturé" /><KpiCard label="PROCHAIN" value={data.summary.nextCheckpointAt} delta="checkpoint backend" /></section>
      <section className="operator-grid operator-grid--top" aria-label="Chronologie live">
        <Card title="Planifié vs réalisé" eyebrow="AUTHORITATIVE TIMELINE" density="compact"><DataTable columns={timelineColumns} rows={data.events.slice(0, 6)} rowKey={(row) => row.eventId} caption="Chronologie live" emptyLabel="Aucun événement de timeline publié." /></Card>
        <Card title="Lecture séquentielle" eyebrow="CYCLE" density="compact"><Timeline events={data.events.slice(0, 5).map((event) => ({ id: event.eventId, title: event.title, at: event.actualAt ?? event.plannedAt, description: `${event.status} · ${event.summary}`, tone: tone(event.status) }))} /></Card>
        <Card title="Scope courant" eyebrow="SESSION" density="compact"><div className="reconciliation-grid"><MetricBox label="Session" value={data.scope.sessionId} /><MetricBox label="Date" value={data.scope.tradingDate} /><MetricBox label="Mode" value={data.scope.mode} /><MetricBox label="Disponibilité" value={meta.availability ?? "AVAILABLE"} /></div></Card>
      </section>
      <section className="operator-grid operator-grid--bottom" aria-label="Détails timeline"><Card title="À traiter" eyebrow="NEXT" density="compact">{data.events.filter((event) => ["WAITING", "SCHEDULED", "READY"].includes(event.status.toUpperCase())).slice(0, 5).map((event) => <div className="detail-list-row" key={event.eventId}><div><strong>{event.title}</strong><small>{event.summary}</small></div><StatusBadge tone="info">{event.status}</StatusBadge></div>)}</Card><Card title="Latence" eyebrow="SLO" density="compact"><div className="operational-detail-list">{data.events.filter((event) => event.latencySeconds != null).slice(0, 5).map((event) => <article key={event.eventId}><div><strong>{event.title}</strong><small>{event.plannedAt}</small></div><b>{durationSeconds(event.latencySeconds)}</b></article>)}</div></Card><Card title="Vérité temporelle" eyebrow="NO SYNTHETIC EVENTS" density="compact"><p>Les événements absents ne sont pas reconstruits dans le navigateur. La timeline affiche uniquement les checkpoints et analyses publiés par le backend.</p></Card></section>
    </div>
  );
}
...
export function OperationsObservabilityPage() {
  ...
  const { data, meta } = query.data;
  return (
    <div className="operator-page operations-observability-page"><ViewTruthBanner meta={meta} /><OperatorPageHeader title="Observabilité" description="Files, workers IA, latences, consommation et violations SLO mesurées." actions={<Link className="operator-primary-action" to="/operations">Operations Hub</Link>} />
      <section className="operator-kpi-strip" aria-label="Indicateurs observabilité"><KpiCard label="PROCESS" value={`${data.summary.processes}`} delta={`${data.summary.running} running`} /><KpiCard label="QUEUE" value={`${data.queue.depth}`} delta={durationMs(data.queue.oldestQueuedMs)} tone={data.queue.depth ? "warning" : "success"} /><KpiCard label="ÉCHECS" value={`${data.summary.failed}`} delta={`${data.summary.retries} retries`} tone={data.summary.failed ? "danger" : "neutral"} /><KpiCard label="SLO" value={`${data.summary.slaBreaches}`} delta="violations" tone={data.summary.slaBreaches ? "warning" : "success"} /><KpiCard label="P95" value={durationMs(data.summary.p95ExecutionMs)} delta="exécution" /><KpiCard label="WORKERS" value={`${data.workerSummary.healthy}/${data.workerSummary.expected}`} delta="healthy / attendus" tone={data.workerSummary.healthy >= data.workerSummary.expected && data.workerSummary.expected > 0 ? "success" : "warning"} /></section>
      <section className="operator-grid operator-grid--top" aria-label="Process, workers et queue"><Card title="Process observés" eyebrow="RUNTIME" density="compact"><DataTable columns={processColumns} rows={data.processes.slice(0, 6)} rowKey={(row) => row.processId} caption="Process observés" emptyLabel="Aucun process observé." /></Card><Card title="Workers IA" eyebrow="FLEET" density="compact"><DataTable columns={workerColumns} rows={data.workers.slice(0, 6)} rowKey={(row) => row.workerId} caption="Workers IA" emptyLabel="Aucun worker enregistré dans la télémétrie." /></Card><Card title="Queue & leases" eyebrow="CAPACITY" density="compact"><div className="reconciliation-grid"><MetricBox label="Depth" value={data.queue.depth} /><MetricBox label="Plus ancien" value={durationMs(data.queue.oldestQueuedMs)} /><MetricBox label="Leases actifs" value={data.queue.activeLeases} /><MetricBox label="Expirants" value={data.queue.expiringLeases} /><MetricBox label="Expirés" value={data.queue.expiredLeases} /><MetricBox label="Running" value={data.summary.running} /></div></Card></section>
      <section className="operator-grid operator-grid--bottom" aria-label="Breakdown, runtime et couverture"><Card title="Par workflow" eyebrow="BREAKDOWN" density="compact"><DataTable columns={workflowColumns} rows={data.workflowBreakdown.slice(0, 6)} rowKey={(row) => row.label} caption="Breakdown par workflow" emptyLabel="Aucun breakdown workflow publié." /></Card><Card title="Runtime IA" eyebrow="SETTINGS" density="compact"><div className="reconciliation-grid"><MetricBox label="Reasoning" value={data.runtimeSettings.reasoningEffort} /><MetricBox label="Révision" value={data.runtimeSettings.revision} /><MetricBox label="Source" value={data.runtimeSettings.source} /><MetricBox label="Application" value={data.runtimeSettings.appliesTo} /><MetricBox label="Tokens" value={nullable(data.summary.totalTokens)} /><MetricBox label="Coût" value={data.summary.costUsd == null ? "—" : `$${data.summary.costUsd.toFixed(4)}`} /></div></Card><Card title="Couverture" eyebrow="TELEMETRY" density="compact"><Coverage coverage={data.coverage} /><p>{meta.warnings?.length ? meta.warnings.join(" · ") : "Aucune source BFF signalée indisponible."}</p></Card></section>
    </div>
  );
}
...
const macroColumns = [
  ...
  { key: "forecast", header: "Forecast", render: (row: LiveNewsView["macroEvents"][number]) => row.forecast },
  { key: "actual", header: "Actual", render: (row: LiveNewsView["macroEvents"][number]) => row.actual }
] as const;

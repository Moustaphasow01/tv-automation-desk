...
export function IncidentsOperationsPanel({ incidents, operations }: { incidents: CommandCenterView["incidents"]; operations: CommandCenterView["operations"] }) {
  return (
    <CommandPanel code="A" title="Incidents & opérations" className="cc-panel--incidents" action={<Link to="/execution/incidents">Voir tout</Link>}>
      {incidents.length ? <div className="cc-table-scroll" role="region" aria-label="Incidents défilables" tabIndex={0}><table className="cc-table"><thead><tr><th>Sévérité</th><th>Détecté</th><th>Ressource</th><th>Problème</th><th>Runbook</th><th>Action</th></tr></thead><tbody>{incidents.map((incident) => <tr key={incident.id}><td><PanelStatus tone={statusTone(incident.severity)}>{incident.severity}</PanelStatus></td><td>{displayTime(incident.detectedAt)}</td><td>{incident.resource}</td><td><Link to={`/operations/incidents/${encodeURIComponent(incident.id)}`}>{incident.title}</Link></td><td>{incident.runbook}</td><td>{incident.action}</td></tr>)}</tbody></table></div> : <EmptyPanelState label="Aucun incident ouvert dans la projection" />}
      <footer className="cc-incident-stats"><span>Files: <strong>{displayNumber(operations.queuedTasks)}</strong></span><span>DLQ: <strong>{displayNumber(operations.dlqItems)}</strong></span><span>Flux périmés: <strong>{displayNumber(operations.staleFeeds)}</strong></span></footer>
    </CommandPanel>
  );
}

    ...
    <CommandPanel code="A" title="Marché & fraîcheur des données" className="cc-panel--market">
      {market.rows.length ? (
        <div className="cc-table-scroll" role="region" aria-label="Flux marché défilable" tabIndex={0}><table className="cc-table"><thead><tr><th>Instrument</th><th>Source</th><th>asOf</th><th>Fraîcheur</th><th>Statut</th></tr></thead>
          <tbody>{market.rows.map((row) => <tr key={row.id}><td><strong>{row.instrument}</strong><small>{row.timeframe}</small></td><td>{row.source}</td><td>{displayTime(row.asOf)}</td><td>{displayDecimal(row.freshnessSeconds, "s")}</td><td><PanelStatus tone={statusTone(row.status)}>{row.status}</PanelStatus></td></tr>)}</tbody>
        </table></div>
      ) : <EmptyPanelState status="SOURCE NON PUBLIÉE" label="Aucun feed marché n'est disponible dans la projection autoritaire." />}
      <div className="cc-freshness-summary"><span>Fraîcheur globale</span><strong>{displayDecimal(market.freshnessSeconds, "s")}</strong><PanelStatus tone={statusTone(market.status)}>{market.status}</PanelStatus></div>
    </CommandPanel>
  );
}
  ...
  return (
    <CommandPanel code="B" title="Pipeline de recherche" className="cc-panel--research" action={<Link to="/research">Voir tout</Link>}>
      ...
      {research.rows.length ? <div className="cc-table-scroll" role="region" aria-label="Pipeline de recherche défilable" tabIndex={0}><table className="cc-table"><thead><tr><th>Mission</th><th>Dataset(s)</th><th>Run</th><th>Statut</th><th>Workers</th><th>Artefacts</th></tr></thead>
        <tbody>{research.rows.map((row) => <tr key={row.id}><td><Link to={`/research/experiments/${encodeURIComponent(row.id)}`}>{row.mission}</Link></td><td>{row.dataset}</td><td>{row.run}</td><td><PanelStatus tone={statusTone(row.status)}>{row.status}</PanelStatus></td><td>{displayNumber(row.workers)}</td><td>{displayNumber(row.artifacts)}</td></tr>)}</tbody>
  ...
  return (
    <CommandPanel code="C" title="Signaux & file de risque" className="cc-panel--signals" action={<Link to="/live/signals">Voir tout</Link>}>
      {signals.rows.length ? <div className="cc-table-scroll" role="region" aria-label="Signaux et risque défilables" tabIndex={0}><table className="cc-table"><thead><tr><th>Heure</th><th>Instrument</th><th>Setup</th><th>Conf.</th><th>Gate</th><th>Port. Arb.</th><th>Risque</th></tr></thead>
        <tbody>{signals.rows.map((row) => <tr key={row.id}><td>{displayTime(row.at)}</td><td><strong>{row.instrument}</strong></td><td>{row.setup}</td><td>{displayDecimal(row.confidence)}</td><td>{row.gate}</td><td><PanelStatus tone={statusTone(row.portfolioDecision)}>{row.portfolioDecision}</PanelStatus></td><td><PanelStatus tone={statusTone(row.riskDecision)}>{row.riskDecision}</PanelStatus></td></tr>)}</tbody>
      ...
      {selected ? <div className="cc-signal-detail"><span>Signal affiché: <strong>{selected.instrument} — {selected.setup}</strong></span><PanelStatus tone={statusTone(selected.riskDecision)}>{selected.riskDecision}</PanelStatus><dl><div><dt>Confiance</dt><dd>{displayDecimal(selected.confidence)}</dd></div><div><dt>Gate</dt><dd>{selected.gate}</dd></div><div><dt>Portfolio</dt><dd>{selected.portfolioDecision}</dd></div><div><dt>Risque</dt><dd>{selected.riskDecision}</dd></div></dl></div> : null}
    </CommandPanel>
  );
}

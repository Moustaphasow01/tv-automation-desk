...
export function FinancialChart({ bars, instrument, overlay, annotations, pricePrecision = 2 }: { bars: readonly ChartBar[]; instrument: string; overlay: TradeOverlay | null; annotations: ChartAnnotations; pricePrecision?: number }) {
  ...
  const { container, navigate } = useFinancialChart({ bars, overlay: visiblePlan.overlay, vwap: vwap ? "shown" : "hidden", annotations, pricePrecision });
  const last = bars.at(-1);
  return <>
    <div className="tw-chart__ohlc" role="group" tabIndex={0} aria-label={`Dernière bougie ${instrument}`}>
      {([['O', last?.open], ['H', last?.high], ['B', last?.low], ['C', last?.close]] as const).map(([label, value]) => <span key={label}>{label} <b>{numberLabel(value, pricePrecision)}</b></span>)}
    ...
    {showData ? <WorkspaceDialog title={`${instrument} · bougies reçues`} onClose={() => setShowData(false)}><CandleTable bars={bars} pricePrecision={pricePrecision} /></WorkspaceDialog> : null}
  </>;
}

function CandleTable({ bars, pricePrecision }: { bars: readonly ChartBar[]; pricePrecision: number }) {
  ...
  return <><p>Valeurs reçues du desk, sans interpolation. Heures de Paris. « — » signifie non publié.</p>
    <div className="tw-table-scroll" tabIndex={0} role="region" aria-label="Tableau des bougies">
      <table><caption>{rows.length} bougies affichées sur {bars.length}</caption><thead><tr>{["Heure", "Ouverture", "Haut", "Bas", "Clôture", "Volume"].map((label) => <th key={label} scope="col">{label}</th>)}</tr></thead>
        <tbody>{[...rows].reverse().map((bar) => <tr key={bar.time}><th scope="row">{parisTime(new Date(bar.time * 1_000).toISOString(), true)}</th>{[bar.open, bar.high, bar.low, bar.close, bar.volume].map((value, index) => <td key={index}>{numberLabel(value, index === 4 ? 2 : pricePrecision)}</td>)}</tr>)}</tbody></table>

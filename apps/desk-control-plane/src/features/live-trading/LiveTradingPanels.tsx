...
export function MacroSessionPanel({ model }: { model: LiveTradingModel }) {
  return <LivePanel title="Macro / Session" className="lt-panel--macro"><dl className="lt-definition-list"><Pair label="Session" value={model.source.session.activeSession ?? model.source.session.phase} /><Pair label="Trading date" value={model.source.session.tradingDate} /><Pair label="Market state" value={model.source.session.marketState ?? model.source.session.marketDataStatus} /><Pair label="Timezone" value={model.source.session.exchangeTimezone ?? "—"} /><Pair label="Last known" value={displayTime(model.source.session.lastKnownAt)} /><Pair label="Signal cutoff" value={displayTime(model.freshness.signalCutoffAt)} /></dl><p className="lt-panel-note">Macro/news: {model.source.macroSession ? String(model.source.macroSession.availability ?? "KNOWN") : "UNAVAILABLE"}</p></LivePanel>;
}
...
export function ProviderRuntimePanel({ model }: { model: LiveTradingModel }) {
  ...
  return <LivePanel title="Provider Runtime" className="lt-panel--provider" action={<span>{provider ? provider.label : "No provider state"}</span>}><ol className="lt-provider-steps">{lifecycle.map((step) => <li key={step.stepId}><i aria-hidden="true" /><span>{step.label} · {step.status}</span></li>)}</ol><TruthEmpty when={!lifecycle.length} label="Aucun lifecycle provider canonique publié dans le mode courant." /><footer><span>ACKNOWLEDGEMENT IS NOT A FILL.</span>{provider ? <StatusBadge tone={liveTone(provider.status)}>{provider.status}</StatusBadge> : <StatusBadge tone="warning">DISABLED BY POLICY</StatusBadge>}</footer></LivePanel>;
}

export function ReconciliationPanel({ model }: { model: LiveTradingModel }) {
  return <LivePanel title="Position Reconciliation" className="lt-panel--reconciliation"><div className="lt-reconciliation"><section><h3>Expected State</h3>{model.reconciliation.expected ? <pre>{compactRecord(model.reconciliation.expected)}</pre> : <TruthEmpty label="Aucun état théorique canonique publié." />}</section><section><h3>Broker State</h3>{model.reconciliation.broker ? <pre>{compactRecord(model.reconciliation.broker)}</pre> : <TruthEmpty label="Aucun snapshot broker réconcilié publié." />}</section><aside><strong>{model.reconciliation.status}</strong><span>{model.reconciliation.detail}</span><small>asOf {displayTime(model.reconciliation.asOf)}</small></aside></div></LivePanel>;
}
...
function TruthEmpty({ when = true, label }: { when?: boolean; label: string }) { return when ? <div className="lt-empty"><FaTimes aria-hidden="true" /><strong>CONNECTED EMPTY</strong><span>{label}</span></div> : null; }

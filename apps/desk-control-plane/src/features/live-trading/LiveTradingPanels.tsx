...
export function ProviderRuntimePanel({ model }: { model: LiveTradingModel }) {
  ...
  return <LivePanel title="Provider Runtime" className="lt-panel--provider" action={<span>{provider ? provider.label : "No provider state"}</span>}><ol className="lt-provider-steps">{lifecycle.map((step) => <li key={step.stepId}><i aria-hidden="true" /><span>{step.label} · {step.status}</span></li>)}</ol><TruthEmpty when={!lifecycle.length} status="DÉSACTIVÉ PAR POLITIQUE" label="L'exécution physique est fermée ; aucun lifecycle provider n'est attendu." /><footer><span>ACKNOWLEDGEMENT IS NOT A FILL.</span>{provider ? <StatusBadge tone={liveTone(provider.status)}>{provider.status}</StatusBadge> : <StatusBadge tone="warning">DISABLED BY POLICY</StatusBadge>}</footer></LivePanel>;
}

export function ReconciliationPanel({ model }: { model: LiveTradingModel }) {
  return <LivePanel title="Position Reconciliation" className="lt-panel--reconciliation"><div className="lt-reconciliation"><section><h3>Expected State</h3>{model.reconciliation.expected ? <pre>{compactRecord(model.reconciliation.expected)}</pre> : <TruthEmpty status="AUCUNE POSITION THÉORIQUE" label="Aucune position théorique canonique n'est ouverte." />}</section><section><h3>Broker State</h3>{model.reconciliation.broker ? <pre>{compactRecord(model.reconciliation.broker)}</pre> : <TruthEmpty status="NON APPLICABLE" label="L'exécution physique est désactivée ; aucun snapshot broker n'est attendu." />}</section><aside><strong>{model.reconciliation.status}</strong><span>{model.reconciliation.detail}</span><small>asOf {displayTime(model.reconciliation.asOf)}</small></aside></div></LivePanel>;
}

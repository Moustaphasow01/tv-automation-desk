        ...
        <aside className="signal-dossier-sidebar" aria-label="Contexte et actions du dossier">
          <DossierSection eyebrow="Provenance" title="Identité canonique" icon={<FaFingerprint aria-hidden="true" />} compact>
            ...
            <nav className="signal-related-links" aria-label="Objets liés">
              <Link to={`/strategies/${encodeURIComponent(data.identity.strategyId)}`}><FaProjectDiagram aria-hidden="true" /> Stratégie</Link>
              <Link to={data.arbitration.portfolioRoute}><FaBalanceScale aria-hidden="true" /> Portefeuille</Link>
...
function RelatedRecords({ data }: { data: LiveSignalDetailView }) {
  return <div className="signal-related-records"><section><h3>Positions théoriques / broker</h3>{data.existingPositions.length ? data.existingPositions.map((position) => <article key={position.positionId}><div><strong>{position.symbol} {position.side}</strong><small>{position.positionId}</small></div><span>{position.quantity} · {formatSignedR(position.pnlR)}</span></article>) : <EvidenceEmpty title="Aucune position liée" detail="État vide confirmé pour ce signal." />}</section><section><h3>Ordres liés</h3>{data.linkedOrders.length ? data.linkedOrders.map((order) => <article key={order.orderId}><div><strong>{order.side} {order.quantity} · {order.type}</strong><small>{order.orderId}</small></div><StatusBadge tone={order.state === "FILLED" || order.state === "ACKED" ? "success" : order.state === "REJECTED" ? "danger" : "warning"}>{presentBackendStatus(order.state).label}</StatusBadge></article>) : <EvidenceEmpty title="Aucun ordre lié" detail="Le signal n’a pas produit d’ordre dans cette projection." />}</section></div>;
}

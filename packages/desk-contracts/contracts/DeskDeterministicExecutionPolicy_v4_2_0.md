# DeskDeterministicExecutionPolicy — v4.2.0

## 1. Objet

Policy V4.2 définit la frontière entre analyse GPT et décisions déterministes pour LIVE et REPLAY. Elle s’applique à Master V5.2, Monitor V2.2, Execution Plan V1.2, Monitor Command V1.2 et Condition Catalog V1.2.
...
- jusqu’à cinq candidats distincts classés, sans qu’un candidat secondaire invalide annule automatiquement le principal ;
...
Seuls les prédicats du Catalog V1.2 sont exécutables. Les paramètres sont lus dans `condition.parameters`; un fallback plat ne sert qu’à la compatibilité legacy. Une condition inconnue ne devient jamais vraie implicitement.
...
La prose n’est jamais exécutable. Seuls les enums, opérateurs et `parameters` du Catalog V1.2 alimentent le moteur. Une source requise absente vaut `UNKNOWN`; pour un événement obligatoire, cela bloque uniquement `ENTRY_TRIGGER`. Les données sont chargées selon le timeframe de chaque condition, sans fallback silencieux vers M1. Une séquence atomique cassure/retest peut porter ses preuves de zone et rejet : ces composantes restent auditées, mais ne constituent plus des veto redondants une fois la séquence satisfaite. Une confirmation ne peut déclencher une entrée que sur une bougie M1 fermée ultérieure, et une réacquisition doit être réévaluée après toute sortie de zone.
...
Les runs déjà épinglés V5.1/V2.1/V1.1/Policy V4.1, V5.0/V2.0/V1.0/Policy V4.0, ainsi que V4/V1/Policy V3, restent lisibles via l’adaptateur legacy et ne sont jamais réinterprétés silencieusement avec V5.2/V2.2/V1.2/Policy V4.2. Toute nouvelle activation est versionnée et hash-lockée.

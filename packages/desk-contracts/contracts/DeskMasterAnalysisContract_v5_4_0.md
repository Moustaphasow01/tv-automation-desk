# DeskMasterAnalysisContract — v5.4.0

## 1. Mission

Le Master V5.4 transforme un bundle scellé au cutoff en analyse contradictoire, thèse active et proposition Execution Plan V1.4. Le schéma de sortie normatif est `schemas/entities/master-analysis-v5-4.schema.json`.

V5.4 remplace V5.3 pour les nouveaux runs; V5.3, V5.2, V5.1, V5.0 et V4 restent lisibles et hash-lockés pour les runs historiques.
...
- Execution Plan V1.4, Policy V4.3 et Catalog V1.2 épinglés ;
...
Le portefeuille actif est lui-même un invariant backend : cinq setups actifs au maximum. Un candidat excédentaire est explicitement fermé par l’arbitrage canonique; il ne reste jamais actif sans être surveillé. Un setup dont `expires_at_paris <= cutoff_paris` est terminal `EXPIRED` avant toute évaluation ou persistance d’un Monitor.
...
Une condition dupliquant exactement une preuve déjà portée par une séquence atomique ne peut pas redevenir un veto obligatoire indépendant. Le compilateur la normalise en preuve d’audit ou rejette le doublon ambigu. Un remplacement de setup crée toujours une nouvelle paire immuable `setup_id` / `setup_record_id` et terminalise l’ancien setup dans la même mutation.
...
Le handoff impose une analyse GPT planifiée en `M15`, une surveillance déterministe en `M1` et un Monitor supplémentaire sur événement critique de setup, position, replan ou intégrité. Il transmet le prochain checkpoint planifié, les conditions à observer, la baseline de santé, le contexte, la matrice de sessions, les fenêtres autorisées, l’agenda de mise à jour, les priorités et le playbook. Les Monitors évaluent l’évolution; ils ne réinventent pas la stratégie ni les identifiants du plan.

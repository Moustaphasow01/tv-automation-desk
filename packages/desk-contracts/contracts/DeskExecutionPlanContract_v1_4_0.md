# DeskExecutionPlanContract — v1.4.0

## 1. Statut et portée

Ce contrat définit la proposition d’exécution structurée produite par le Master GPT **avant** compilation déterministe. Le schéma normatif est `schemas/entities/execution-plan-v1-4.schema.json`.
...
Le plafond de cinq s’applique aussi à l’état persistant, pas seulement au JSON du Master. À chaque Monitor, le backend expire d’abord les setups arrivés à échéance, applique atomiquement les remplacements, puis conserve au plus cinq setups actifs. Tout excédent reçoit un état terminal auditable.
...
Une preuve de prix strictement équivalente à une composante déjà absorbée par `BREAK_RETEST_SEQUENCE` ne peut pas être ajoutée comme seconde condition obligatoire indépendante. Le compilateur la déduplique vers une preuve advisory ou rejette la définition si son intention est ambiguë.
...
- analyse GPT événementielle : dernier `M1` fermé lors d’une transition critique ;
- anti-lookahead vérifié : `true` ;
- contextes épinglés : Master V5.4, Plan V1.4, Policy V4.3, Catalog V1.2.

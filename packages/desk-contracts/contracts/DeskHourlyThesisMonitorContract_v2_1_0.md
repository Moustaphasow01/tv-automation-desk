# DeskHourlyThesisMonitorContract — v2.1.0

## 1. Mission et cadence

Le Monitor V2.1 réévalue la thèse et le plan à chaque checkpoint GPT `M5`, tandis que le moteur déterministe évalue les conditions et la position en `M1`. Le nom historique « Hourly » est conservé pour compatibilité d’API; il ne définit plus la cadence.

Le schéma normatif est `schemas/entities/hourly-monitor-v2-1.schema.json`. V2.0 et V1 restent lisibles et hash-lockés pour les runs historiques.
...
`command` est conforme à DeskMonitorCommandContract V1.1. Les transitions de thèse, setup, position et replan sont orthogonales. `ARM` signifie armer conditionnellement; seul le moteur peut exécuter `ENGINE_TRIGGER`.
...
Le Monitor prouve l’anti-lookahead, les versions Monitor V2.1 / Command V1.1 / Policy V4.1 / Catalog V1.1 et les références de source. Toute propriété inconnue est rejetée.
